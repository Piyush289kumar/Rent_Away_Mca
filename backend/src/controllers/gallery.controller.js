...
export const createGallery = async (req, res) => {
  try {
    ...
    ================================ */
    if (!req.files?.galleryMedia?.[0]) {
      return res.status(400).json({ message: "Gallery image is required." });
    }

    const file = req.files.galleryMedia[0];

    const upload = await uploadToCloudinary(file.path, "gallery/media");
...
export const updateGallery = async (req, res) => {
  try {
    ...
    if (req.files?.galleryMedia?.[0]?.path) {
      ...
      gallery.thumbnail = upload.secure_url;
